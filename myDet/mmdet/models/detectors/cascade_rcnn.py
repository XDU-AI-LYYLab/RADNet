...
from mmdet.core import (bbox2result, bbox2roi, bbox_mapping, build_assigner,
                        ...
                        multiclass_nms, anchor_offset) #-----------------添加了anchor offset
from .. import builder
...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None,
                 num_rpn_stages=1): #-------------添加 num_rpn_stages
    ...
    @property
    def with_rpn(self):
        return (hasattr(self, 'rpn_head') and self.rpn_head is not None and self.num_rpn_stages == 1)
        #------要添加 self.num_rpn_stages == 1 的判断
    ...
    def init_weights(self, pretrained=None):
        ...
        #------------------修改 cascade rpn 的 初始化---------------
        elif self.with_cascade_rpn:
            for i in range(self.num_rpn_stages):
                self.rpn_head[i].init_weights() #------------每一个rpn阶段都需要初始化-------
        #--------------------------------------------------------
    ...
    def forward_train(self,
        ...
        #-------------添加 cascade_rpn -----------------------------------------
            ...
            anchor_list, valid_flag_list = self.rpn_head[0].init_anchors(featmap_sizes, img_meta)
            for i in range(self.num_rpn_stages):
                rpn_train_cfg = self.train_cfg.rpn[i]
                rpn_head = self.rpn_head[i]

                if rpn_head.feat_adapt:
                    offset_list = anchor_offset(anchor_list, rpn_head.anchor_strides, featmap_sizes)
                else:
                    offset_list = None
                ...
                if i<self.num_rpn_stages-1:
                    anchor_list = rpn_head.refine_bboxes(anchor_list, bbox_pred, img_meta)
            proposal_list = self.rpn_head[-1].get_bboxes(anchor_list, cls_score, bbox_pred, img_meta, self.test_cfg.rpn)
        #------------------------------------------------------------------------
