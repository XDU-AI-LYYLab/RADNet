...
heatmappath = '/home/lyy/hq/DTestRes/paper2/noskip'
...
#--------------------------------------------------de convolution -------------------------------
        ...
        if self.attention:
            #=================== self attention ==============================
            self.non_local = Attention2D(in_channels=self.out_channels, pooling_type='simple_att',fusion_types=('conv_add',))
    ...
    def forward(self, inputs):
        ...
        # visualize_feature_map(inputs[self.start_level].cpu().numpy(), savename='inputs', savepath=heatmappath)
        # build laterals
        laterals = [
        ...
        '''
        # ---------------------------draw feature map ---------
        # visualize_feature_map(outs[0].cpu().numpy(), savename='noskip', savepath=heatmappath)
        # b, c, h, w = outs[0].shape
        # draw_features(c, w, h, outs[0].cpu().numpy(), savename='noskip', savepath=heatmappath)
        ...
        if self.attention:
            ...
            for i,out in enumerate(outs): # out[0]先下采样，通过注意力后，上采样
                ...
                # print(20*"*")
                #====================== channel attention =============================
                # b, c, width, height = out_tmp.shape
                # y = self.avg_pool(out_tmp).view(b, c)
                ...
                # final = final_c
                    # print(last.shape)
                #======================= position attention=============================
                final = self.non_local(out_tmp)
                # if i>0:
                #     last = self.non_local(final_c)
                #     final_s = last
                # else:
                #     out1_size = outs[i+1].size()[2:]
                #     out0_size = final_c.size()[2:]
                #     temp = F.adaptive_max_pool2d(final_c, output_size=out1_size)
                #     last = self.non_local(temp)
                #     final_s = F.interpolate(last, size=out0_size, mode='nearest')
                # final = final_s
                #==========================================================================
        ...
        #     print(la.shape)
        #--------------------------- after attention --------------------------------------
        # visualize_feature_map(outs[0].cpu().numpy(), savename='after_att', savepath=heatmappath)
        # b, c, h, w = outs[0].shape
        # draw_features(c, w, h, outs[0].cpu().numpy(), savename='after_att', savepath=heatmappath)
        #-----------------------------------------------------------------------------------
